  ...
  const insights = useMemo(() => {
    ...
    const lastMonthExpenses = transactions
      .filter((t) => {
        const date = new Date(t.date);
        return (
          t.type === "expense" && date >= lastMonth && date <= lastMonthEnd
        );
      })
      .reduce((sum, t) => sum + t.amount, 0);
    ...
    if (lastMonthExpenses > 0) {
      ...
      insights.push({
        ...
        } by ${Math.abs(change).toFixed(1)}% compared to last month`,
      ...
      if (currentMonthBudgets.length > 0) {
        ...
        if (overBudgetCategories > 0) {
          insights.push({
            ...
            icon: AlertTriangle,
            title: "Budget Alert",
            description: `You're over budget in ${overBudgetCategories} ${
    ...
    const topCategory = Object.entries(currentMonthByCategory).sort(
      ([, a], [, b]) => b - a
    )[0];
    ...
    const currentMonthTransactions = transactions.filter((t) => {
      const date = new Date(t.date);
      return (
        t.type === "expense" && date >= currentMonth && date <= currentMonthEnd
      );
    });
